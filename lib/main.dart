...
class _PhysihoAppState extends State<PhysihoApp> {
  ...
  _navigateToDetail(Map<String, dynamic> message) {
    if (message['data']['type'] == 'business') {
      launchScreen(context, BusinessView.routeName, arguments: message['data']['id']);
    }
  }
        ...
        onGenerateRoute: (RouteSettings settings) {
          ...
          switch(settings.name) {
            ...
            case DepartmentView.routeName:
              return MaterialPageRoute(builder: (_) => DepartmentView(department: args));
              break;
            // business view
            case BusinessView.routeName:
              return MaterialPageRoute(builder: (_) => BusinessView(businessId: args));
              break;
            // item view
            case ItemView.routeName:
              return PageRouteBuilder(
                pageBuilder: (context, animation, secondaryAnimation) => ItemView(item: args),
            ...
            case CategoryView.routeName:
              return MaterialPageRoute(builder: (_) => CategoryView(categoryName: args));
              break;
            // checkout view
            ...
            case TrackingView.routeName:
              return MaterialPageRoute(builder: (_) => TrackingView(orderId: args));
              break;
            ...
            default:
              return MaterialPageRoute(builder: (context) {
                // Size helper
                  ...
                  } else if (context.watch<LoginProvider>().isLoadingCurrentUser()) {
                    return LoadingView(sourceLoading: "Cargando información del usuario...");
                  } else if (! context.watch<LocationProvider>().isPermissionChecked()) {
